...
import UNIPoolJson from '../../../yam/clean_build/contracts/DrugUniPool.json';
import ZombiePool from '../../../yam/clean_build/contracts/ZombiePool.json';
import CREAMPoolJson from '../../../yam/clean_build/contracts/DrugCREAMPool.json';
import DICEPoolJson from '../../../yam/clean_build/contracts/DrugDICEPool.json';
...
const StatCards: React.FC = () => {
  ...
  const rows = farms.reduce<Farm[][]>((farmRows, farm) => {
    ...
    if (newFarmRows[newFarmRows.length - 1].length) {
      if(farm.sort === 3 || farm.sort === 1 || farm.sort === 0 || farm.id === 'cream' || farm.id === 'Drug' || farm.id === 'dice' ||farm.id === 'taco' || farm.id === 'comp' || farm.id === 'yfi' || farm.id === 'weth') {
       } else {
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  const get_prices_wrapped = useCallback(async (num) => {
    //seconds step
    axios.get('https://api.coingecko.com/api/v3/simple/price?ids=Cethereum%2Cdrug-finance&vs_currencies=usd').then((res) => {
      if (res.status === 200) {
        setCurrentPrice(Number(res.data['Drug-finance'].usd))
        if (yam) {
          gettots()
          getdaistaked(num, Number(res.data['drug-finance'].usd))
          getUserStakedDai()
          getUserEarnedDai()
        }
      }
  ...
  return (
    <>
        <StyledCardWrapper>
          <Card>
            <CardContent>
              ...
              <br />
        ========== PRICES ==========<br />
              {currentPrice && `DRUG: $${thousands_separators(Number(currentPrice))}`}<br />
              ...
              {farm.id === 'uni' && `WETH_DRUG_UNI_LP: $${currentCoinPrice === '1' ? thousands_separators(Number(totalwrapped / totalDai)) : thousands_separators(Number(currentstatPrice))}`}
        ...
        {farm.id === 'zombie'
         ...
         <br/>
        ========== STAKING =========<br />
              {/* Total supply of DRUG-{totalZom}<br/> */}
