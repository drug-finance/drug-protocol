...
const FarmCards: React.FC = () => {
  ...
  const rows = farms.reduce<Farm[][]>((farmRows, farm) => {
    ...
    if (newFarmRows[newFarmRows.length - 1].length === 3) {
      if (farm.name === "Sushi swap" || farm.name === "Taco Tuesday" || farm.name === "Bal_Drug_Dai_95" || farm.name === "Bal_Drug_Dai_80" || farm.name === "Zombie Swamp" || farm.name === "DogeFi Days"){
      } else {
            ...
            <StyledContent>
              ...
              {Date.now() > endTime * 1000 ? (
                <>
                  <Button
                    disabled={!poolActive}
                    text={poolActive ? (farm.name === "WETH_DRUG_UNI_LP" ? 'Select' : 'Remove Liquidity' ) : undefined}
                    to={`/farms/${farm.id}`}
                  />
                </>
              )
                ...
                {farm.name === "WETH_DRUG_UNI_LP" ?
                  <>
                  <br />
                    <StyledDetail>4494.92 Drug/Weekly</StyledDetail>
